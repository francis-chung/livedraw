...
async function saveCanvas(supabase, userId, name, objects, canvasId = null) {
  ...
  if (!existingData) {
    const { error: insertError } = await supabase.from('canvas_data').insert({
      canvas_id: canvas.id,
      objects,
    });
    if (insertError) {
    ...
    sharedCanvasIds.length > 0
      ? (
          await supabase
            .from('canvases')
            .select('id, name, owner_id')
            .in('id', sharedCanvasIds)
        ).data || []
      : [];

  const canvasIds = [...ownedIds, ...sharedCanvasIds];
...
async function shareCanvas(supabase, ownerId, canvasId, targetUserId, role) {
  ...
  const { error: upsertError } = await supabase.from('canvas_access').upsert(
    {
      canvas_id: canvasId,
