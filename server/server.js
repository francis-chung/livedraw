...
io.on('connection', async (socket) => {
  ...
  if (socket.handshake.auth?.accessToken) {
    const verifiedUser = await verifySupabaseToken(
    ...
    if (verifiedUser) {
      ...
      console.log(
        'authenticated via handshake:',
        socket.user.email || socket.user.name || socket.user.id
      );
    } else {
  ...
  socket.on('authenticate', async ({ accessToken }) => {
    ...
    console.log(
      'authenticated user:',
      verifiedUser.email || verifiedUser.name || verifiedUser.id
    );
  ...
  socket.on('updateObjects', (updatedObjects) => {
    ...
    canvasStates[key] = objects.map((obj) => updates.get(obj.id) || obj);
  ...
  socket.on(
    'saveCanvas',
    async ({ name, objects: clientObjects, canvasId }) => {
      if (!requireAuth(socket)) return;
      try {
        const supabase = socket.supabase;
        const objectsToSave = clientObjects || [];
        const savedCanvasId = await saveCanvas(
          supabase,
          socket.user.id,
          name,
          objectsToSave,
        ...
        console.log(
          `Canvas saved as ${name} (${savedCanvasId}) for user ${socket.user.id}`
        );
      } catch (error) {
    ...
    try {
      ...
      if (loaded !== null) {
        ...
        if (canvasId) {
          ...
          console.log(
            `Canvas ${loaded.name} (${canvasId}) loaded in room ${socket.currentRoom} for user ${socket.user.id}`
          );
        } else {
  ...
  socket.on('getSavedCanvases', async () => {
    ...
    try {
      ...
      socket.emit(
        'savedCanvasesError',
        error?.message || 'Unable to retrieve saved canvases'
      );
    }
  });
  ...
  socket.on('shareCanvas', async ({ canvasId, targetUserId, role }) => {
    ...
    try {
      ...
      console.log(
        `Canvas ${canvasId} shared with ${targetUserId} as ${role} by ${socket.user.id}`
      );
    } catch (error) {
...
server.on('error', (error) => {
  if (error.code === 'EADDRINUSE') {
    console.error(
      `Port ${PORT} is already in use. Try stopping the other process or set PORT to a different value.`
    );
    process.exit(1);
  }
  throw error;
